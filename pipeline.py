...
def _build_pair(arg: Argument, rival_arg: Optional[Argument], depth: int,
    ...
    logger.info("%s Arbiter done | node_allowed=%s (will_spawn=%s)  rival_allowed=%s (will_spawn=%s)",
                ...
                arbiter_result.rival_allowed, rival_node.allowed_to_spawn if rival_node else "n/a")
    logger.debug("%s Arbiter reasoning: %s", tag, short(arbiter_result.reasoning, 200))
    ...
    if on_event:
        on_event({"type": "arbiter_done", "path": path, "depth": depth, "data": {
            ...
            "node_spawned_claims": [c.claim for c in arbiter_result.node_claims],
            "rival_spawned_claims": [c.claim for c in arbiter_result.rival_claims],
        }})
    ...
    for n, claims, label, child_prefix in [
        (node,       arbiter_result.node_claims,  "node",  "n"),
        (rival_node, arbiter_result.rival_claims,  "rival", "r"),
    ]:
        if n is None:
            continue
        ...
        if not n.allowed_to_spawn:
            reason = "depth cap" if (arbiter_result.node_allowed if n is node else arbiter_result.rival_allowed) else "arbiter stopped"
